...
function ProtectedRoutesRec({ Component }) {
  ...
  useEffect(() => {
    if (employee_state) {
      axios
        .get("allikhwa-hms/receptionists/" + employee_state)
        .then((res) => {
          if (res.data.employee_name) {
            console.log(res.data.employee_name);
            dispatch(employee_loggedin(res.data));
          } else {
            throw "The receptionist is not found please provide valid doctor email";
          }
        })
        .catch((err) => {
          console.log(err);
...
export default ProtectedRoutesRec;
